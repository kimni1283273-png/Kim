...
equations, the case of degenerate equations with the Euler--Bessel operator 
particularly in the time-fractional setting  remains relatively unexplored.
...
fractional Gronwall inequality \cite{Podlubny}.
...
\begin{proof}
 ...
 Given an arbitrary $\hat{q} \in C([0,T])$, it follows from Theorem~\ref{4.1} that
...
The continuous dependence result of Theorem~\ref{4.1} yields that
...
$w \in C^1([0,l])$ with $w(0)=w(l)$ and $g \in C^1([0,T])$ with $g(t) \neq 0$ for all $t \in [0,T]$.
...
\bibitem{Ye2007}
H.~Ye, J.~Gao, Y.~Ding,
\textit{A generalized Gronwall inequality and its application 
to a fractional differential equation},
J.\ Math.
