...
Elliptik tipdagi tenglamalar uchun chegaraviy masalalarni yechishda (Manba funksiya) Grin funksiyasi muhim o'rin tutadi. Bo'laklari silliq $S$ sirt bilan chegaralangan $D$ sohani qaraylik.\\
4.1-ta'rif. (4.2) Laplas tenglamasi uchun $D$ sohada ichki (tashqi) Dirixle masalasining Grin funksiyasi deb, ikkita $x, \xi \in D \cup S$ nuqtalarning funksiyasi bo'lgan va quyidagi shartlarni qanoatlantiruvchi $G(x, \xi)$ funksiyaga aytiladi [15]:
...
Bu ta'rifga asosan, $G(x, \xi)$ funksiya $\xi$ nuqtadan tashqari barcha $D$ sohada garmonik bo lib, $g(x, \xi)$ funksiya yordamida aniqlanadi. $g(x, \xi)$ funksiya esa o'z navbatida $D$ sohada garmonik bo'lib, $S$ da $-\ln (1 / r), n=2$ yoki $-r^{2-n}, n>2$ qiymatlarga teng. Bu yerda $g(x, \xi)$ shunday garmonik funksiyaki, u chegarada maxsus qiymatlarni qabul qiladi. Ayrim hollarda bunday funksiyani topish ancha qulay bo'ladi. Bu funksiya ma'lum bo'lgandan so'ng, chegarada ixtiyoriy qiymatni qabul qiluvchi garmonik funksiyani topish mumkin bo ladi.

\section*{Grin funksiyasining xossalari:}
\begin{enumerate}
  \item $D \cup S$ da $G(x, \xi) \geq 0$;
  \item agar $D$ sohaning $S$ chegarasi yetarlicha silliq sirt bo lsa, u holda $G(x, \xi)$ funksiya $S$ chegarada har bir $\xi \in D$ da to'g'ri normal $\frac{\partial G(x, \xi)}{\partial n_{x}}$ hosilaga ega;
...
o'rinli. $\left|S_{1}\right|$ - birlik sferaning yuzi.\\
Agar $w=w(z)$ funksiya $D(n=2$ bo'lganda sohani birlik doiraga (ya'ni $|x|<1 g a$ ) konform akslantiruvchi funksiya bo'lsa, u holda Grin funksiyasi quyidagi formuladan
...
\begin{center}
  ...
  \coordinate (y) at (22:1.35);
  \coordinate (ystar) at (22:3.91);
  \coordinate (x) at (72:\R);
  \draw[->] (O) -- (22:4.4);
  \draw (O) -- (x);
  \draw (x) -- (y);
  \draw (x) -- (ystar);
  \fill (O) circle (1.3pt) node[below left] {$O$};
  \fill (y) circle (1.3pt) node[above left] {$y$};
  \fill (ystar) circle (1.3pt) node[above right] {$y^{*}$};
  \fill (x) circle (1.3pt) node[above] {$x$};
  \node at (72:1.25) [left] {$R$};
  \node at (-12:2.55) {$S_{R}$};
\end{tikzpicture}
...
Agar $x \in S_{R}$ bo'lsa, u holda 4.1 - chizmaga ko'ra $O x y{ }^{*}$ va $O x y$ uchburchaklar o'xshash bo'ladi, chunki ular umumiy $O$ burchakka ega va bu burchakni hosil qilgan tomonlari (4.58) va (4.59) tengliklarga ko'ra proporsionaldir, ya'ni
...
tengliklarni hosil qilamiz. Grin funksiyasining ta'rifiga ko'ra $\alpha$ sonini shunday tanlaymizki, $\left.G(x, y)\right|_{S_{R}}=0$ bo'lsin. Bundan va (4.60) dan
...
ega bo"lamiz.\\
Shunday qilib, (4.60) va (4.63)ga ko'ra
...
ni hosil qilamiz.\\
(4.64) formulaning ikkinchi qo'shiluvchisini (4.59) ga asosan quyidagicha ifodalab olamiz:
...
(4.65) ifodani (4.64) formulaga qo'yib, shar uchun Grin funksiyasini quyidagi ko'rinishda topamiz:
...
2-masala. $\Delta u(x, y, z)=0$ tenglamaning $D_{1}: z>0$ yarim fazodagi Grin funksiyasi $G(x, y, z ; \xi, \eta, \zeta)=\frac{1}{4 \pi}\left(\frac{1}{r}-\frac{1}{r_{1}}\right)$ ko rinishga ega bo'lishini ko'rsating, bu yerda
...
ko'rinishga ega bo'ladi. $g(x, y, z ; \xi, \eta, \zeta)$ funksiya, qaralayotgan sohada garmonik funksiya bo'lgani uchun $\Delta g(x, y, z)=0$ va $\Delta g(\xi, \eta, \zeta)=0$ tenglamalarni qanoatlantiradi, shu bilan birga (4.57)\\
shartga asosan $(x, y, z)$ va $(\xi, \eta, \zeta)$ nuqtalarning hech bo'lmaganda bittasi $\partial D_{1}$ ga tegishli bo'lsa, $G(x, y, z, \xi, \eta, \zeta)=0$, ya'ni $g(x, y, z ; \xi, \eta, \zeta)=-1 / r$ bo lishi kerak. Demak, $(x, y, z)$ va $(\xi, \eta, \zeta)$ nuqtalarning hech bo'lmaganda bittasi $\partial D_{1}$ ga tegishli bo'lishi uchun $z=0$ yoki $\zeta=0$ bo'lishi shart. Bundan esa $r=r_{1}$ bo'ladi, ya'ni $g(x, y, z ; \xi, \eta, \zeta)=-1 / r_{1}$. Bu funksiya $D_{1}$ sohada garmonikdir. Shunday qilib,
...
\begin{center}
  ...
  \fill[gray!12] (-3,-0.5) -- (3,-0.5) -- (4,0.6) -- (-2,0.6) -- cycle;
  \draw (-3,-0.5) -- (3,-0.5) -- (4,0.6) -- (-2,0.6) -- cycle;
  \node at (3.5,0.05) {$z=0$};
  \coordinate (M0) at (0.6,2.2);
  \coordinate (M1) at (0.6,-2.2);
  \coordinate (P)  at (-1.6,1.15);
  \fill (M0) circle (1.3pt) node[above right] {$M_{0}(\xi,\eta,\zeta)$};
  \fill (M1) circle (1.3pt) node[below right] {$M_{1}(\xi,\eta,-\zeta)$};
  \fill (P)  circle (1.3pt) node[left] {$M$};
  \draw (P) -- (M0) node[midway,above] {$r$};
  \draw (P) -- (M1) node[midway,left] {$r_{1}$};
  \draw[dashed] (M0) -- (M1);
\end{tikzpicture}
\captionsetup{labelformat=empty}
\caption{4.2 - chizma}
...
\begin{center}
  ...
  \coordinate (M0) at (1.5,1.9);
  \coordinate (M1) at (1.5,-1.9);
  \coordinate (P)  at (-1.7,0.95);
  \fill (M0) circle (1.3pt) node[above right] {$M_{0}(\xi,\eta)$};
  \fill (M1) circle (1.3pt) node[below right] {$M_{1}(\xi,-\eta)$};
  \fill (P)  circle (1.3pt) node[above left] {$M(x,y)$};
  \draw (P) -- (M0) node[midway,above] {$r_{MM_{0}}$};
  \draw (P) -- (M1) node[midway,below left] {$r_{MM_{1}}$};
  \draw[dashed] (M0) -- (M1);
\end{tikzpicture}
\captionsetup{labelformat=empty}
\caption{4.3 - chizma}
...
Integral ifoda [11], [15]. Agar $u(x)$ funkasiya $C^{2}(D) \cap C^{1}(\bar{D})$ sinfga tegishli bo'lsa, u holda bu funksiyaning integral ifodasi
...
Bu funksiyani $\eta$ bo'yicha differensiallab, quyidagiga ega bo'lamiz:
...
Bu funksiyani $\zeta$ bo'yicha differensiallab, so'ng $\zeta=0$ qoyib, quyidagiga ega bo'lamiz:
...
ni hosil qilamiz, ifodadagi qolgan uchta integral, integral ostidagi funksiyalarning toqligiga ko'ra nolga teng bo'ladi.
...
ga ega bo'lamiz.\\[0pt]
(4.82) formulaga chegirmalar haqidagi Koshi [14] teoremasini qo'llaymiz:
...
Shunday qilib, $u(x, y)=\frac{x}{x^{2}+(y+\alpha)^{2}}$ funksiya yuqori yarim tekislikda garmonikdir. Bundan, (4.78) chegaraviy shartga ko'ra $\alpha=1$ ga teng bo'lib, $u(x, y)=\frac{x}{x^{2}+\left(1+y^{\prime}\right)^{2}}$ funksiya (4.73), (4.78) masalaning yechimi bo'ladi.

2-izoh. Agar $z=x+i y, \frac{y}{(x-\xi)^{2}+y^{2}}=\operatorname{Re} \frac{1}{i(\xi-z)}$ bo'lsa, u holda, (4.77) formulani quyidagi ko'rinishda
...
yozib olamiz.\\
Endi yarim tekislikda (Im $z>0$ ) (ya'ni $y>0$ da) Laplas tenglamasi uchun Dirixle masalasini ko'raylik:
...
bu yerda $R(z)$ haqiqiy o'qda qutb maxsus nuqtaga ega bo'lmagan haqiqiy ratsional funksiya bo'lib, $z \rightarrow \infty$ da $R(z) \rightarrow 0$.
...
Dirixle masalasini yeching.\\
Yechish. (4.84) va (4.85) formulalardan foydalanib, Dirixle masalasining yechimini quyidagi ko'rinishda topamiz:
...
ko'rinishda bo'ladi.\\
Endi $c_{1}$ va $c_{2}$ o'zgarmaslarni chegaraviy shartlar yordamida topamiz. (4.86) yechimda $c_{1}=0$, chunki agar $c_{1} \neq 0$ bo'lsa, u holda $z \rightarrow+\infty$ da $\vartheta(z) \rightarrow \infty$ bo'ladi. Shuning uchun (4.86) yechimda $z=0 \operatorname{deb}, 0=\vartheta(0)=c_{2}+2 \Rightarrow c_{2}=-2$ aniqlaymiz. Demak, (4.86) ga ko'ra qo yilgan masalaning yechimi
...
ega bo'lamiz. Bundan xulosa qilib, shuni aytish lozimki
...
Bigarmonik tenglama. Ushbu $\Delta^{2} u=0$ bigarmonik va $\Delta^{2} u=f$ tenglamalar uchun Grin funksiyasidan foydalanmay, chegaraviy masalalarni o'rganish mumkin.
...
ni hosil qilamiz. IV bobning 3 -§iga ko'ra oxirgi integral Laplas tenglamasi uchun qo'yilgan
...
Dirixle masalasining yechimidir. Bu masalaning yechimi $v=r / a \cos \varphi$ ko'rinishda bo'ladi. Demak, qo'yilgan masala yechimining yagonaligiga ko'ra qo'yidagi tenglikka
...
normal.\\
Yechish. Faraz qilaylik, izlanayotgan yechim faqat $r$ ga bog'liq bo'lsin, ya'ni $u=u(r)$. U holda
...
Bu tenglamaning yechimini $u(x, y)=\omega(y) \sin x$ ko'rinishda izlaymiz, $\quad \omega(y)$-noma'lum funksiya. $\quad u(x, y)=\omega(y) \sin x$ funksiyani berilgan tenglamaga qo'yib,
...
ko'rinishda bo'ladi. Masala shartiga ko'ra $C_{1}=0, C_{2}=0$ teng, aksincha, $\quad y \rightarrow+\infty \quad$ da $\quad \omega(y) \rightarrow \infty \quad$ bo'ladi. $\quad C_{3}, \quad C_{4} \quad$ lar $\omega(0)=\omega^{\prime}(0)=0$ shartlardan topiladi. Demak, oddiy differensial\\
...
ko'rinishga ega bolishini ko'rsating. Bu yerda
...
ko'rinishga ega bo'lishini ko'rsating. Bu yerda
...
\begin{enumerate}
  ...
  \item $\Delta u=0$ tenglamaning radiusi $R=4$ ga teng bo'lgan shardagi
\end{enumerate}
...
ko'rinishga ega bo'lishini ko'rsating.\\
...
\begin{enumerate}
  ...
  \item $x^{2}+y^{2}+z^{2}<R^{2}, \quad y>0 \quad z>0$ sharning to'rtdan bir qismida:
...
\begin{enumerate}
  \item $u_{0}$ - bo'lakli-uzluksiz va chegaralangan funksiya;
